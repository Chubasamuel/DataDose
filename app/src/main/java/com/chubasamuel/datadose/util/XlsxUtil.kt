...
object XlsUtil {
    ...
    fun exportFilledOptionsToXlsx(context: Context, project_id:Int, os: BufferedOutputStream, dao: AppDao, isByValue:Boolean):Flow<WorkStatus>{
        return callbackFlow {
            trySend(WorkStatus.Working("Exporting project.."))
        val scope=CoroutineScope(Dispatchers.IO)
        os.let{
            scope.launch {
            val wb= Workbook(it,"DataDose",context.packageManager.getPackageInfo(context.packageName,0).versionName.replace("-debug",""))
            ...
            val indOnlyQues=dao.getAllIndexOnlyNumbers(project_id)
                for(i in indOnlyQues){
                    try{ ws.value(0,i,"Q$i")}catch(e:Exception){
                    /*e.printStackTrace()*/}
                }
                   data.collect{
                          li->  for(d in li){
